...
historical <- nc_open('wam_EC_EARTH_historical_r9i1p1_swh_yearmean.nc')
{
  sink('wam_EC_EARTH_historical_r9i1p1_swh_yearmean.txt')
  print(historical)
  sink()
}
...
zeebrugge_EC_EARTH_df <- data.frame(year= seq(from=1971, to=1999, by=1), SWH=zeebrugge_series)
ggplot(data=zeebrugge_EC_EARTH_df, aes(x=year, y=SWH, group=1)) +
...
#rcp45
rcp45 <- nc_open('wam_EC_EARTH_rcp45_r9i1p1_swh_yearmean.nc')
{
  sink('wam_EC_EARTH_rcp45_r9i1p1_swh_yearmean.txt')
  print(rcp45)
  sink()
}
...
zeebrugge_rcp45_EC_EARTH_df <- data.frame(year= seq(from=2070, to=2100, by=1), SWH_rcp45=t(zeebrugge_series_rcp45))
ggplot(data=zeebrugge_rcp45_EC_EARTH_df, aes(x=year, y=SWH_rcp45, group=1)) +
...
#rcp85
rcp85 <- nc_open('wam_EC_EARTH_rcp85_r9i1p1_swh_yearmean.nc')
{
  sink('wam_EC_EARTH_rcp85_r9i1p1_swh_yearmean.txt')
  print(rcp85)
  sink()
}
...
zeebrugge_rcp85_EC_EARTH_df <- data.frame(year= seq(from=2070, to=2100, by=1), SWH_rcp85=t(zeebrugge_series_rcp85))
ggplot(data=zeebrugge_rcp85_EC_EARTH_df, aes(x=year, y=SWH_rcp85, group=1)) +
  geom_line(aes(color = 'RCP-8.5'))+   # Set title
  theme_bw()+
  ylim(2, 5)+ylab('Significant waveheight (m)') + 
  geom_line(data = zeebrugge_rcp45_EC_EARTH_df, aes(y = SWH_rcp45, color = 'RCP-4.5'))+
  geom_smooth(method=lm, aes(color = 'RCP-8.5')) + 
  geom_smooth(data = zeebrugge_rcp45_EC_EARTH_df, method = lm, aes(y = SWH_rcp45, color = 'RCP-4.5'))+
