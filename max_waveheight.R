...
historical <- nc_open('wam_MIROC5_historical_r5i1p1_swh_yearmax.nc')
{
  sink('wam_MIROC5_historical_r5i1p1_swh_yearmax.txt')
  print(historical)
  sink()
}
...
zeebrugge_df <- data.frame(year= seq(from=1971, to=1999, by=1), SWH=zeebrugge_series)
ggplot(data=zeebrugge_df, aes(x=year, y=SWH, group=1)) +
  geom_line(color = "black") + # make this a line plot
  ggtitle("Max Significant Waveheight Historical") +     # Set title
  theme_bw()+
  ylim(10, 22) + ylab('Significant waveheight (m)') + 
...
#rcp45
rcp45 <- nc_open('wam_MIROC5_rcp45_r3i1p1_swh_yearmax.nc')
{
  sink('wam_MIROC5_rcp45_r5i1p1_swh_yearmax.txt')
  print(rcp45)
  sink()
}
...
zeebrugge_rcp45_df <- data.frame(year= seq(from=2070, to=2100, by=1), SWH_rcp45=t(zeebrugge_series_rcp45))
ggplot(data=zeebrugge_rcp45_df, aes(x=year, y=SWH_rcp45, group=1)) +
...
#rcp85
rcp85 <- nc_open('wam_MIROC5_rcp85_r3i1p1_swh_yearmax.nc')
{
  sink('wam_MIROC5_rcp85_r5i1p1_swh_yearmax.txt')
  print(rcp85)
  sink()
}
...
zeebrugge_rcp85_df <- data.frame(year= seq(from=2070, to=2100, by=1), SWH_rcp85=t(zeebrugge_series_rcp85))
ggplot(data=zeebrugge_rcp85_df, aes(x=year, y=SWH_rcp85, group=1)) +
  geom_line(aes(color = 'RCP-8.5'))+   # Set title
  theme_bw()+
  ylim(10, 22)+ylab('Significant waveheight (m)') + 
  geom_line(data = zeebrugge_rcp45_df, aes(y = SWH_rcp45, color = 'RCP-4.5'))+
  geom_smooth(method=lm, aes(color = 'RCP-8.5')) + 
  geom_smooth(data = zeebrugge_rcp45_df, method = lm, aes(y = SWH_rcp45, color = 'RCP-4.5'))+
  labs(title = "Max Significant Waveheight RCP-4.5 and RCP-8.5",
       color = "Scenario's")+ 
